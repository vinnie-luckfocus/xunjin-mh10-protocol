# Xunjin MH10 Modbus RTU 通信协议规范 V1.6.0
...
| 0x1D | MH10_MB_REG_PROTOCOL_VERSION | RO | 0x0160 | 协议版本 V1.6.0 |
...
| 0x6C | MH10_MB_FO_CURVE_SUPPORT_RO | RO | 0xC0DE | 曲线调节支持标识：支持本功能的固件固定返回 MH10_CURVE_SUPPORT_MAGIC（0xC0DE）；旧固件（REG_COUNT=0x50）读该地址返回非法地址异常（0x02），主机据此优雅降级（隐藏/禁用曲线调节功能） |
| 0x6D~0x6F | 保留 | - | - | 保留 |
