# Xunjin MH10 Modbus RTU 通信协议规范 V1.11.0
...
| 0x1D | MH10_MB_REG_PROTOCOL_VERSION | RO | 0x01B0 | 协议版本 V1.11.0（低字节高半字节非 BCD 数字，按 16 位原始值比较） |
...
| 0x47 | MH10_MB_FO_BOOST_CURRENT_RW | RW | 0 | 连续模式启动助力峰值电流（V1.11.0，0.1A）：连续旋转模式起步/换向/升速时峰值电流先给本值，爬坡到位并稳定 0x17 时长后恢复曲线查表正常电流；0=用驱动器型号上限（522→22/556→26），有效范围 1~26，固件按探测到的 DM2C 型号钳上限；往复模式不使用，后板不实现 |
...
### 5.2.2 前板正/反转电流曲线调节区（V1.6.0 新增，V1.10.0 起为固定分段布局，0x50~0x98）

供 box 系统维护"正反转校准"页面对连续旋转模式的 速度→峰值电流 分段阶梯曲线在线调参。
曲线模型（V1.10.0 起）：不再有可变速度阈值，速度域按每 **500 rpm** 一个固定分段，共
**18 段**（工具头输出轴最大转速 9000 rpm = 500×18）。段 i 覆盖转速区间
**[500×i, 500×(i+1))**，查表 `seg = min((uint32_t)(speed/500), 17)`，无插值，纯阶梯。
共四套曲线（驱动器型号 × 旋转方向）：DM2C522 正转 / DM2C522 反转 / DM2C556 正转 /
DM2C556 反转，运行时按实际检测到的 DM2C 型号与当前方向选一套。
每套曲线占 **18 个寄存器**（基址 + 0..17），全部为峰值电流 CUR[0..17]，单位 0.1A。
写入时固件全局钳制到 1~26（0.1A~2.6A），再按曲线所属型号钳制上限：
**522 → 22**（DM2C-RS522 峰值 2.2A），**556 → 26**（DM2C-RS556 允许放宽到 2.6A）。
写寄存器立即生效（易失，不擦写 flash，持久化由 box 侧 sys.ini 负责）。

| 地址 | 名称 | 方向 | 默认值 | 说明 |
|------|------|------|--------|------|
| 0x50+k | MH10_MB_FO_CURVE_FWD_522_BASE | RW | 见下 | DM2C522 正转曲线（0x50~0x61）：k=0~17 峰值电流 CUR[k]（0.1A），段 k 覆盖转速 [500k, 500(k+1)) rpm |
| 0x62+k | MH10_MB_FO_CURVE_REV_522_BASE | RW | 见下 | DM2C522 反转曲线（0x62~0x73），布局同上 |
| 0x74+k | MH10_MB_FO_CURVE_FWD_556_BASE | RW | 见下 | DM2C556 正转曲线（0x74~0x85），布局同上 |
| 0x86+k | MH10_MB_FO_CURVE_REV_556_BASE | RW | 见下 | DM2C556 反转曲线（0x86~0x97），布局同上 |
| 0x98 | MH10_MB_FO_CURVE_SUPPORT_RO | RO | 0xC0DE | 扩展区支持标识：支持 0x50~0x9B 扩展区（电流曲线 + 调试直驱，同一固件版本一并实现）的固件固定返回 MH10_CURVE_SUPPORT_MAGIC（0xC0DE）；旧固件读该地址返回非法地址异常（0x02），主机据此优雅降级（隐藏/禁用曲线调节与调试直驱功能） |

默认值（正/反转相同，共 18 段；保持 V1.9.x 旧 3 阈值 + 4 档阶梯形状的等价映射——
旧 speed≤1000 对应新 CUR[0..1]、≤4000 对应 CUR[2..7]、≤7000 对应 CUR[8..13]、
否则对应 CUR[14..17]）：

- 522（0.1A）：`{16,16, 18,18,18,18,18,18, 20,20,20,20,20,20, 22,22,22,22}`
- 556（0.1A）：`{18,18, 20,20,20,20,20,20, 23,23,23,23,23,23, 25,25,25,25}`

**兼容性（Break）**：本布局与 V1.9.x（3 阈值 + 4 档电流，曲线区 0x50~0x6B、支持标识
0x6C、调试直驱区 0x6D~0x6F）**不兼容**——V1.9.x 及更早固件（REG_COUNT=0x70）读写
0x62 以外的新曲线地址返回非法地址异常（0x02）。主机须先读协议版本寄存器 0x1D
（本布局固件为 0x01A0）并读 0x98 支持标识魔数，两者共同判定新布局是否可用。

### 5.2.3 前板调试直驱寄存器（V1.7.0 新增于 0x6D~0x6F，V1.10.0 迁移至 0x99~0x9B）

供 box 系统维护"调试模式"页面直接驱动电机连续旋转，完全独立于切割/往复状态机与
调参手动档位运行（不占用 0x20~0x4F 调参区任何寄存器）。启动后电机按 0x99 设定转速、
0x9A 方向持续连续旋转，直到收到停止命令；电流按 5.2.2 曲线区对应方向/驱动器型号的
曲线随速度下发。支持判定与曲线区共用 0x98 支持标识，旧固件写 0x99~0x9B 返回非法
地址异常（0x02），主机据此优雅降级。

| 地址 | 名称 | 方向 | 默认值 | 说明 |
|------|------|------|--------|------|
| 0x99 | MH10_MB_FO_DEBUG_SPEED_RW | RW | 0 | 直驱设定转速（工具头输出轴 rpm，与 0x0B 同刻度，固件钳制到安全范围） |
| 0x9A | MH10_MB_FO_DEBUG_DIR_RW | RW | 0 | 直驱方向：0=正转 1=反转 |
| 0x9B | MH10_MB_FO_DEBUG_CMD_WO | WO | 0 | 直驱命令：1=启动（按 0x99/0x9A 连续旋转） 2=停止（减速停机），执行后清零 |
...
仅在 bootloader 运行时有效，与 app 模式寄存器是相互独立的命名空间。bootloader 同时应答系统寄存器 0x18（0xA0A0）、0x19（复位魔数）、0x1D（0x01A0），并将 0x1B 报为 0x0000（用于触发主控板版本不一致判定）。
...
| V1.10.0 | 2026/09/10 | 前板速度→峰值电流曲线扩展区改为**每 500 rpm 一个固定分段**的新布局（Break）：取消 3 个可变速度阈值，曲线区改为 18 段峰值电流 CUR[0..17]（段 i 覆盖转速 [500i, 500(i+1))，查表 seg=min(speed/500,17)，纯阶梯；单位仍 0.1A，全局钳制 1~26，按型号上限 522→22 / 556→26）；四套曲线（DM2C522/DM2C556 × 正/反转）各占 18 寄存器，基址 0x50/0x62/0x74/0x86；支持标识迁至 0x98（魔数不变 0xC0DE）；调试直驱区自 0x6D~0x6F 迁至 0x99~0x9B（区内布局不变）；REG_COUNT 0x70 → 0x9C。默认值保持 V1.9.x 旧阶梯形状等价映射（522：16,16,18×6,20×6,22×4；556：18,18,20×6,23×6,25×4）。与 V1.9.x 布局不兼容，靠协议版本 0x1D=0x01A0 与 0x98 魔数区分（V1.9.x 及更早固件读写新地址返回非法地址异常 0x02）。注：V1.10.0 起 MINOR=10，0x1D 值 0x01A0 低字节高半字节为 0xA（非 BCD 数字），主机按 16 位原始值比较版本 |
| V1.11.0 | 2026/09/10 | 新增前板寄存器 0x47（MH10_MB_FO_BOOST_CURRENT_RW）连续模式启动助力峰值电流：单位 0.1A，连续旋转模式起步/换向/升速时峰值电流先给本值保证遇阻启动力矩，爬坡到位并稳定 0x17 时长后恢复曲线查表正常电流；0（上电默认）= 用驱动器型号上限（522→22 / 556→26），有效范围 1~26，固件按探测到的 DM2C 型号钳上限；写入立即生效，易失不擦 flash（持久化由 box 侧 sys.ini 负责）；仅连续旋转模式使用，往复模式/后板不实现；REG_COUNT 保持 0x9C |
