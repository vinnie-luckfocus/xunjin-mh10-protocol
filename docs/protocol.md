# Xunjin MH10 Modbus RTU 通信协议规范 V1.8.0
...
| 0x1D | MH10_MB_REG_PROTOCOL_VERSION | RO | 0x0180 | 协议版本 V1.8.0 |
...
仅在 bootloader 运行时有效，与 app 模式寄存器是相互独立的命名空间。bootloader 同时应答系统寄存器 0x18（0xA0A0）、0x19（复位魔数）、0x1D（0x0180），并将 0x1B 报为 0x0000（用于触发主控板版本不一致判定）。
